...
(if (inl-p (nvle_int (funcall nvx_) (funcall nvx)))
(funcall nvy)
(let ((dec (funcall nvy)))
(if (inl-p dec)
(let ((nvl (inl-val dec)))
(if (inl-p (nvlt_int (funcall nvx_) (funcall nvldrs_uid
(lambda () nvl))))
(make-inl :val nvh)
(funcall nvy)))
(if (inr-p dec)
...
(lambda (nvloc)
(funcall (lambda (nvx)
(nveclass2-program (nveclass1-program (funcall nvx) (nvaneris_pax_v1_react_base-program nvCmd nvdecision nvpropose nvmf)) (funcall (nvaneris_pax_v1_MonitorState-program nvCmd nvdecision nvpropose nvmf)
(lambda () (funcall nvloc)))))
(lambda () (lambda (nvldr)
(lambda (nvz)
(declare (ignore nvz))
...
(funcall (lambda (nvx)
(nveclass2-program (nveclass1-program (funcall nvx) (nvaneris_pax_v1_p1a_base-program nvCmd nvdecision nvpropose nvmf)) (nvaneris_pax_v1_AcceptorState-program nvCmd nvdecision nvldrs_uid nvpropose nvmf)))
(lambda () (lambda (nvloc)
(lambda (nvzh)
(lambda (nvz)
...
(funcall (lambda (nvx)
(nveclass2-program (nveclass1-program (funcall nvx) (nvaneris_pax_v1_p2a_base-program nvCmd nvdecision nvpropose nvmf)) (nvaneris_pax_v1_AcceptorState-program nvCmd nvdecision nvldrs_uid nvpropose nvmf)))
(lambda () (lambda (nvloc)
(lambda (nvzi)
(lambda (nvz)
...
(nvmissing (pair-snd pair)))
(funcall (lambda (nvx)
(nvband (nvle_int (funcall nvx) nvmax) (nvbnot (funcall (funcall (nvaneris_batch_is_in_missing)
(lambda () (funcall nvx)))
(lambda () nvmissing)))))
(lambda () (funcall (nvaneris_batch_command_tag2cid nvOp)
...
(lambda (nvp)
(nvbor (let ((pair (funcall nvx_)))
(let ((nvslt (pair-fst pair))
(nvcmdts (pair-snd pair)))
(declare (ignore nvslt))
(if (inl-p (nvisl nvcmdts))
(nveq_int (funcall nvx) (funcall (nvaneris_batch_protocol2num)
(lambda () (nvoutl nvcmdts))))
(nvreduce (lambda (nvx_)
(lambda (nvp)
(nvbor (nveq_int (funcall nvx) (funcall (nvaneris_batch_command2num nvOp)
(lambda () (funcall nvx_)))) (funcall nvp)))) (nvbfalse) (nvoutr nvcmdts))))) (funcall nvp)))) (nvbfalse) (funcall nvtodo)))
(lambda () (funcall (nvaneris_batch_command_tag2num nvOp)
(lambda () (funcall nvcmdt))))))))
...
(make-pair :fst (make-pair :fst nva :snd (make-pair :fst nvb :snd (make-pair :fst nvatimer_ :snd (make-pair :fst (funcall nvx) :snd (make-pair :fst nve :snd (make-pair :fst nvf :snd nvg)))))) :snd nvmsgs))))
(lambda () (funcall (funcall (nvaneris_batch_add_proposal_if_not_in nvOp)
(lambda () (funcall nvcmdt)))
(lambda () nvd))))))))))))))))))))))
...
(lambda (nvproposals)
(funcall (lambda (nvx)
(if (inl-p (nvisl (funcall nvx)))
(make-inl :val (nvoutl (funcall nvx)))
(funcall (lambda (nvx)
(make-inr :val (funcall nvx)))
(lambda () (funcall (fix (lambda (nvlist_ind)
(lambda (nvL)
(let ((nvv (funcall nvL)))
(if (pair-p nvv)
(let ((pair nvv))
...
(error "bottom")))))))
(lambda () (funcall nvproposals)))))))
(lambda () (nvhd (funcall nvproposals))))))
...
(make-pair :fst (make-pair :fst nva :snd (make-pair :fst nvb :snd (make-pair :fst nvc :snd (make-pair :fst nvd :snd (make-pair :fst nve :snd (make-pair :fst nvf :snd nvg)))))) :snd (make-axiom))
(funcall (lambda (nvx)
(make-pair :fst (make-pair :fst nva :snd (make-pair :fst (nvbtrue) :snd (make-pair :fst nvc :snd (make-pair :fst nvd :snd (make-pair :fst nve :snd (make-pair :fst nvf :snd nvg)))))) :snd (funcall nvx)))
(lambda () (if (inl-p nvb)
(make-axiom)
...
(lambda () nvpax_procs))
(lambda () (make-pair :fst nva :snd (funcall nvx)))))
(lambda () (funcall (nvaneris_batch_get_batch nvOp)
(lambda () nvd))))
(if (inl-p (funcall (funcall (nvlist-deq (nvatom-deq))
(lambda () nvg))
...
(lambda (nvprotocol)
(funcall (lambda (nvx)
(make-pair :fst (funcall nvx) :snd (funcall (funcall (nvaneris_batch_response_broadcast nvOp nvbcast nvdecision nvpax_propose nvresponse nvswap nvtimer nvtt_propose nvmf)
(lambda () nvclients))
(lambda () (make-pair :fst (funcall nvslot_num) :snd (funcall nvcmdts))))))
(lambda () (let ((dec (funcall nvcmdts)))
(if (inl-p dec)
(let ((nvz (inl-val dec)))
(let ((pair nvz))
(let ((nvcid (pair-fst pair))
...
(nvmissing (pair-snd pair)))
(funcall (lambda (nvx)
(if (inl-p (nvlt_int nvmax (funcall nvx)))
(make-pair :fst (funcall nvx) :snd (nvappend nvmissing (nvfrom-upto (+ nvmax 1) (funcall nvx))))
(if (inl-p (nveq_int (funcall nvx) nvmax))
(make-pair :fst nvmax :snd nvmissing)
...
(lambda () nve)))
(make-pair :fst (make-pair :fst nva :snd (make-pair :fst nvb :snd (make-pair :fst nvc :snd (make-pair :fst nvd :snd (make-pair :fst nve :snd (make-pair :fst (funcall nvx) :snd nvg)))))) :snd (make-axiom))
...
(nvmsgs (pair-snd pair)))
(funcall (lambda (nvx_)
(make-pair :fst (make-pair :fst (+ nva 1) :snd (make-pair :fst (nvbfalse) :snd (make-pair :fst nvc :snd (make-pair :fst nvd :snd (make-pair :fst (funcall nvx_) :snd (make-pair :fst (funcall nvx) :snd nvprotocol_)))))) :snd nvmsgs))
(lambda () (funcall (funcall (funcall (nvaneris_batch_add_decisions nvOp)
(lambda () nva))
(lambda () (funcall nvcmdts)))
(lambda () nve))))))))
(lambda () (funcall (funcall (nvaneris_batch_filter_out_todo nvOp)
(lambda () nva))
(lambda () nvf)))))))))))))))))))
...
(- (funcall nvx) (funcall (nvaneris_batch_command_tag2cid nvOp)
...
(lambda () (make-pair :fst nva :snd (make-pair :fst nvb :snd (make-pair :fst nvc :snd (make-pair :fst (funcall nvx_) :snd (make-pair :fst nve :snd (make-pair :fst (funcall nvx) :snd nvg))))))))
(lambda () (funcall nvx)))))
(let ((nvz (pair-fst pair))
...
(lambda () (funcall (funcall (nvaneris_batch_filter_out_proposals nvOp)
(lambda () nvcmdts))
(lambda () nvd)))))
(lambda () (funcall (funcall (funcall (funcall (funcall (nvaneris_batch_add_todo nvOp)
(lambda () nva))
(lambda () nvslt))
(lambda () nvcmdts))
(lambda () nve))
