...
NOTE: I did not have any luck installing the FreeBSD package using
pkg_add, but extracting the archive was enough.  The archive already
contains these three binaries.

NOTE: Do not install the 2013 version.  Some libraries that we use
have been removed from the 2013 version.

3) To generate the Nuprl library file, type 'make lib'.

NOTE: 1), 2), and 3) can be done by simply typing 'make'.

4) To install the SBCL Lisp compiler on FreeBSD, type 'pkg_add -r
sbcl'.  We're using several packages that we've installed using
quicklisp.  Here is how to get quicklisp and install these packages
using quicklisp:
...
NOTE: If you have already installed quicklisp, you just need to do the
ql:quickload's.

The Lisp version of Aneris can be compiled by typing:
  'make aneris-lisp'.
...
'startTobcastClient.sh'. To start Aneris in Lisp mode, use the scripts
'startAnerisX-lisp.sh'.


EXAMPLE
-------

This section explains how to test ShadowDB and Aneris in interpreted
mode locally.  We will use the conf/conf_aneris_local.emlc
configuration file instead of the conf/conf_aneris.emlc file.
